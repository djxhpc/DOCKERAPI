...
JOBS = [
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\112\管線\測量拍照-管頂埋深照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出",
        "job_name":   "112管線-測量拍照-管頂埋深",
    },
    # 新增更多任務範例：
#     {
...
# ─────────────────────────────────────────────────────────────
# STEP 2a  YOLO 尺規 + 細長物體交叉辨識 (合併版)
# ─────────────────────────────────────────────────────────────

# ─────────────────────────────────────────────────────────────
# STEP 2a  YOLO 尺規 + 細長物體交叉辨識 (顏色遮罩加強版)
# ─────────────────────────────────────────────────────────────

def _detect_slender_object_crossing(img_bgr, img_hsv, ruler_bbox):
    """
    圍繞 YOLO 偵測到的尺規範圍，尋找是否有細長物體與其交叉。
    加入「主軸角度分析」，防範平行放置的物體（如木柄）因為落入 BBox 範圍而造成誤判。
    """
    ...
    sx1, sx2 = max(0, x1 - expand_x), min(w_img, x2 + expand_x)
    sy1, sy2 = max(0, y1 - expand_y), min(h_img, y2 + expand_y)

    roi_bgr = img_bgr[sy1:sy2, sx1:sx2]
    roi_hsv = img_hsv[sy1:sy2, sx1:sx2]
    if roi_bgr.size == 0: 
        return False

    lower_yellow = np.array([12, 60, 60], dtype=np.uint8)
    ...
    if contours:
        ...
        if cv2.contourArea(largest_c) > 50:
            # 取得最小包覆矩形
            rect = cv2.minAreaRect(largest_c)
            box_pts = cv2.boxPoints(rect)
            
            # 計算矩形的兩邊長度，找出「長邊」來代表尺規的走向
            d1 = np.hypot(box_pts[0][0] - box_pts[1][0], box_pts[0][1] - box_pts[1][1])
            d2 = np.hypot(box_pts[1][0] - box_pts[2][0], box_pts[1][1] - box_pts[2][1])
            if d1 > d2:
            ...
            # 計算角度並正規化到 0~180 度之間
            r_ang = np.degrees(np.arctan2(dy, dx))
            if r_ang < 0: r_ang += 180
            ruler_angle = r_ang % 180

    # 如果因為污損算不出角度，就用 YOLO BBox 的長寬比盲猜水平或垂直
    if ruler_angle is None:
        ruler_angle = 0.0 if rw > rh else 90.0
    ...
    gray_roi_masked = cv2.bitwise_and(gray_roi, gray_roi, mask=non_yellow_mask)

    edges = cv2.Canny(gray_roi_masked, 20, 80, apertureSize=3)
    min_line_len = max(30, int(min(rw, rh) * 0.8)) 
    lines = cv2.HoughLinesP(edges, 1, np.pi/180, threshold=40, minLineLength=min_line_len, maxLineGap=30)

    if lines is None: 
        return False

    clip_rect = (x1, y1, rw, rh)

    for line in lines[:, 0]:
        lx1, ly1, lx2, ly2 = line
        gx1, gy1 = sx1 + lx1, sy1 + ly1
        gx2, gy2 = sx1 + lx2, sy1 + ly2

        intersects, pt1, pt2 = cv2.clipLine(clip_rect, (gx1, gy1), (gx2, gy2))

        if intersects:
            total_length = np.hypot(gx2 - gx1, gy2 - gy1)
            inside_length = np.hypot(pt2[0] - pt1[0], pt2[1] - pt1[1])
            outside_length = total_length - inside_length

            if outside_length > max(15, min(rw, rh) * 0.2):
                
                # ==========================================
                # 排除平行物體：比對線條與尺規的角度差異
                # ==========================================
                line_angle = np.degrees(np.arctan2(gy2 - gy1, gx2 - gx1))
                if line_angle < 0: line_angle += 180
                line_angle = line_angle % 180

                # 計算銳角差異 (0~90度)
                diff = abs(line_angle - ruler_angle)
                if diff > 90:
                    diff = 180 - diff
                
                # 如果夾角小於 25 度，代表這條線跟尺規幾乎「平行」(例如放在旁邊的木柄)
                # 這種情況不是交叉，直接忽略它！
                if diff < 25:
                    continue
                # ==========================================
...
def _is_valid_ruler_color(img_hsv, bbox):
    """
    雙重確認機制：檢查 YOLO 抓到的 BBox 內，是否有足夠比例的黃色像素。
    防止把電線桿、灰色水管等誤判為尺規。
    """
    ...
    # 只要該框內有超過 5% 的面積是黃色系，我們就承認它是尺規
    return yellow_ratio > 0.05

def run_combined_ruler(output_json_path, folder_path, model_path):
    """
    執行合併版的尺規辨識，輸出單一檔案 ruler_combined_results.json
    包含 4 向旋轉 (0, 90, 180, 270) 偵測，解決模型方向盲區，並修正 RGB/BGR 色彩空間問題。
    """
    from ultralytics import YOLO as YOLOModel

    out_dir = os.path.dirname(output_json_path)
    results_path = os.path.join(out_dir, "ruler_combined_results.json")
    ...
    print(f"  [尺規辨識] 載入模型: {model_path}")
    model = YOLOModel(model_path)
    print(f"  [尺規辨識] 開始處理 {len(new_items)} 張新照片...")
    ...
    for idx, item in enumerate(new_items, 1):
        ...
        try:
            ...
            # 【關鍵修復】將 RGB 轉換為 OpenCV / YOLO 預設的 BGR 格式
            img_bgr_orig = cv2.cvtColor(arr, cv2.COLOR_RGB2BGR)
        ...
        # 準備 4 種旋轉模式 (0度, 順時針90度, 180度, 逆時針90度)
        rotations = [
            None, 
        ...
        for rot in rotations:
            # 依據目前的迴圈轉動圖片
            if rot is not None:
                img_bgr = cv2.rotate(img_bgr_orig, rot)
            else:
                img_bgr = img_bgr_orig.copy()

            # YOLO 推論 (注意：這裡餵入的是 BGR 格式！)
            preds = model.predict(source=img_bgr, verbose=False)[0]
            
            # 將圖片轉成 HSV，供後續顏色驗證與交叉分析使用
            img_hsv = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2HSV)

            valid_boxes = []
            for box in (preds.boxes or []):
                if float(box.conf[0].item()) > 0.15:  # 稍微把下限調到 0.15 減少雜訊
                    bbox = box.xyxy[0].tolist()
                    # 【防呆機制】確認這個框裡面真的有黃色！
                    if _is_valid_ruler_color(img_hsv, bbox):
                        valid_boxes.append(bbox)
            # 只要在這個角度下有抓到尺規，就直接以這個角度做後續的「交叉分析」
            if valid_boxes:
                # 產生 HSV 給交叉檢測用
                
                
                for bbox in valid_boxes:
                    if _detect_slender_object_crossing(img_bgr, img_hsv, bbox):
                        crossed = True
                        break
                
                if crossed:
                    status = "Both Detected (Crossed)"
                    sm["both_detected_crossed"] += 1
                else:
                    status = "Vertical Only"
                    sm["vertical_only"] += 1
                
                # 既然已經找到並分析完畢，就不需要再轉下一個角度了
                break
        
        if not valid_boxes:
            sm["none_detected"] += 1

        rec.update(status=status, detections={"ruler_count": len(valid_boxes)})
        report["results_list"].append(rec)

        if idx % 100 == 0 or idx == len(new_items):
            print(f"  [尺規辨識] 進度: {idx}/{len(new_items)}")

    sm["total_images"] = len(report["results_list"])
    with open(results_path, 'w', encoding='utf-8') as f:
        json.dump(report, f, ensure_ascii=False, indent=4)
    print(f"  [尺規辨識] 完成！結果儲存至: {results_path}")
...
def main():
    ...
    for job in JOBS:
        ...
        for root, dirs, _ in os.walk(source_dir):
            ...
            # Step 2a: 合併版 YOLO 尺規 + 交叉辨識
            if "ruler_combined" in steps and RULER_YOLO_ENABLED:
                run_combined_ruler(output_json, root, YOLO_MODEL_PATH)
