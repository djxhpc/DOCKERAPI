...
```bash
# 建議使用獨立 conda / venv 環境
conda create -n vision_pipeline python=3.11 -y
conda activate vision_pipeline

pip install ultralytics rapidocr_onnxruntime opencv-python pillow numpy imagehash

# 若要使用 FastAPI 服務
pip install fastapi "uvicorn[standard]" python-multipart

# 若要使用 viewer.py（需有 GUI / X11 環境）
# Ubuntu: sudo apt install python3-tk
```

將 `vision_pipeline.py` 中 `paths` 字典裡四個 `.pt` 模型的路徑改成實際路徑：
...
# 處理單張影像
result = vp.process_single_image(
    "/data/photos/IMG_0001.jpg",
    output_dir="/data/output"
)
print(result["class"], result["needs_review"])

# 處理整個資料夾（只處理還沒有對應 json 結果的新照片，不遞迴）
new_count = vp.process_folder_once("/data/photos", output_dir="/data/output")
print(f"新處理 {new_count} 張")
```
...
vp.PHASH_SUSPECTED_THRESHOLD = 8  # 選填，預設 8

result = vp.process_single_image("/data/photos/IMG_0002.jpg", output_dir="/data/output")
if "suspected_duplicate" in result:
    print("疑似重複於:", result["suspected_duplicate"])
```
...
```bash
python viewer.py --output /data/output
```

- 左側顯示原圖預覽，右側顯示分類與各模型辨識摘要。
- 按 `O`（通過）或 `X`（待修正），或用左右鍵切換照片。
- `X` 會在該筆結果 json 加上 `"reviewed": "X"`，並寫入 `<output_dir>/error_outputs.json`。
- `O` 則加上 `"reviewed": "O"`，並從 `error_outputs.json` 移除。
...
- 已移除原先的「常駐輪詢監控」功能；新照片需透過函式庫呼叫 / CLI / API 主動觸發處理。
- 四個 YOLO 模型與 RapidOCR 為延遲載入＋全域快取，同一行程內重複呼叫不會重新載入模型；FastAPI 服務啟動後第一次請求會稍慢（載入模型），之後會變快。
- 若需 GPU，請確保 `ultralytics` 環境可存取 CUDA；模型載入程式碼未強制指定裝置，預設行為依 ultralytics 設定。