...
# 設定區 — 請依實際情況修改
# =============================================
BASE_DIR = r"D:\work2\ocr"
YEARS    = ["一等水準點","測量讀數照片","埋深"]
...
BENCHMARK_YOLO_MODEL_PATH = r"path\to\benchmark_best.pt" # 訓練好的一等水準點偵測模型
...
CLASSIFY_MODEL_PATH  = r"path\to\classify_best.pt" # classify_train.py 訓練輸出的 best.pt
CLASSIFY_CONF_THRESH = 0.6    # 低於此信心值 → 自動 fallback 到 FOLDER_RULES
...
COORD_YOLO_MODEL_PATH = r"path\to\coord_classify.pt"  # 訓練好的座標格式分類模型
COORD_YOLO_CONF       = 0.5   # 低於此信心值 → fallback 到全格式模式（類別 9）
...
COORD_YOLO_CLASS_MAP  = {
    ...
    "local_surface": 4,   # 本地N/E 高程 / 地表N/E/H
    "north_east":    5,   # 北座標 東座標 高（繁簡）/ 北東高度 / 北東高
    "bottom_left":   6,   # 抓左下角 N E H
    "mixed_NEZ":     7,   # N E Z 或 北東高程
    "nez_only":      8,   # N E Z
}
# =============================================
...
def run_coord_ocr(output_json_path, folder_path, group_id=None):
    ...
    if COORD_YOLO_ENABLED:
        from ultralytics import YOLO as YOLOModel
        try:
            coord_yolo = YOLOModel(COORD_YOLO_MODEL_PATH)
    ...
    for idx, item in enumerate(new_items, 1):
        ...
        try:
            ...
            if class_id == 6:
                ...
                if class_id in _CLASS_PATTERNS:
                    res = _match_coord(text, _CLASS_PATTERNS[class_id])
                    # 指定格式未全中 → fallback 到全格式
