...
JOBS = [
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\110\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "110孔蓋開關閥-孔蓋埋深-照片",
    },
   {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\111\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "111孔蓋開關閥-孔蓋埋深-照片",
    },
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\112\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "112孔蓋開關閥-孔蓋埋深-照片",
    },
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\113\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "113孔蓋開關閥-孔蓋埋深-照片",
    },
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\114\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "114孔蓋開關閥-孔蓋埋深-照片",
    },
    {
        "source_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\115\孔蓋開關閥\孔蓋埋深-照片",
        "steps":      ["ruler_combined"],
        "output_dir": r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\新輸出\孔蓋開關閥-孔蓋埋深-照片",
        "job_name":   "115孔蓋開關閥-孔蓋埋深-照片",
    },
    # 新增更多任務範例：
#     {
...
# ── 工具路徑 ──────────────────────────────────
YOLO_MODEL_PATH    = r"C:\Users\WF_114.WFUSION\Desktop\pin\Chiayi\117\尺規bestv3.pt"
...
# ─────────────────────────────────────────────────────────────
# STEP 2a  YOLO 尺規交叉辨識 (純 YOLO 判斷版)
#   類別假設：
#       class 'vertical'   -> 垂直尺規 / 主物件
#       class 'horizontal' -> 水平物件
#   判斷邏輯：
#       None Detected          : 完全沒偵測到 vertical
#       Vertical Only          : 有 vertical，無有效 horizontal (conf 容許到 0.2)
#       Normal (Crossed)       : vertical 與 horizontal BBOX 有交叉
#       Both Detected (Not Crossed) : 兩者都有但 BBOX 沒接觸
# ─────────────────────────────────────────────────────────────

# horizontal 的最低信心門檻：低於此值視為「沒偵測到 horizontal」
...
def _get_class_name(model, cls_id):
    """安全取得類別名稱（小寫）"""
    try:
        return model.names[int(cls_id)].lower()
    except Exception:
        return str(cls_id)
...
def _boxes_intersect(box_a, box_b):
    ...
    判斷兩個 BBOX 是否有交叉（重疊）。
    box 格式: [x1, y1, x2, y2]
    回傳: (is_intersect: bool, iou: float)
    """
...
def _classify_crossing(vertical_boxes, horizontal_boxes):
    """
    根據 vertical / horizontal BBOX 判斷狀態。

    判斷邏輯：
      None Detected    : 完全沒偵測到 vertical（horizontal 有無皆可）
      Normal (Crossed) : 有 vertical 即視為正常
                         （含「vertical 與 horizontal 交叉」
                           及「只有 vertical、無 horizontal」）
    ...
    回傳:
        ...
        best_iou (float)
    """
    # 1. 沒有 vertical -> None Detected（不論 horizontal 有沒有）
    if not vertical_boxes:
        return "None Detected", False, 0.0

    # 2. 有 vertical 但沒有 horizontal -> 歸為 Normal (Crossed)
    if not horizontal_boxes:
        return "Normal (Crossed)", False, 0.0

    # 3. 兩者都有 -> 計算交叉資訊（仍歸 Normal）
    best_iou = 0.0
    is_crossed = False
...
#       class 'horizontal' -> 水平物件
#   判斷邏輯：
#       None Detected    : 完全沒偵測到 vertical（horizontal conf 容許到 0.2）
#       Normal (Crossed) : 有 vertical 即正常（含交叉、或只有 vertical 無 horizontal）
# ─────────────────────────────────────────────────────────────

def run_combined_ruler(output_json_path, folder_path, model_path):
    """
    尺規辨識主流程（純 YOLO 判斷版 v4）。
    狀態分類：
      - "None Detected"    : 完全沒偵測到 vertical
      - "Normal (Crossed)" : 有 vertical（正常）
      - "Error"            : 讀檔失敗
    """
    ...
    if os.path.exists(results_path):
        with open(results_path, 'r', encoding='utf-8') as f:
            report = json.load(f)
        sm = report["summary"]
        sm.setdefault("normal_crossed", 0)
        sm.setdefault("none_detected", 0)
        sm.setdefault("file_read_errors", 0)
        # 清除舊版統計欄位
        sm.pop("vertical_only", None)
        sm.pop("both_detected_not_crossed", None)
    else:
        report = {
            "summary": {
                "total_images": 0,
    ...
    for idx, item in enumerate(new_items, 1):
        ...
        for box in (preds.boxes or []):
            ...
            cls_name = _get_class_name(model, box.cls[0])

            if cls_name == "vertical":
                if conf >= VERTICAL_CONF_THRESHOLD:
                    vertical_boxes.append(bbox)
            elif cls_name == "horizontal":
